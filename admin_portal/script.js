...
async function viewSocietyDetail(societyId) {
    try {
        ...
        content.innerHTML = `
            ...
            <!-- Central Verification Policy Toggle -->
            <div class="policy-setting-card" style="background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 16px; margin: 20px 0;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1;">
                        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                            <span style="font-size: 20px;">🛡️</span>
                            <h3 style="margin: 0; font-size: 16px; font-weight: 600;">Require Central Verification</h3>
                        </div>
                        <p style="color: #6c757d; font-size: 13px; margin: 0; margin-left: 30px;">
                            Optional policy – when enabled, agents must complete central verification (does not replace Aadhaar verification).
                            <strong style="color: #dc3545;">OFF by default.</strong> For future enforcement only.
                        </p>
                    </div>
                    <div class="toggle-container" style="margin-left: 20px;">
                        <label class="toggle-switch">
                            <input type="checkbox" id="central-verification-toggle" 
                                   ${currentSociety.requireCentralVerification ? 'checked' : ''}
                                   onchange="toggleCentralVerificationPolicy('${currentSociety.societyId}')">
                            <span class="toggle-slider"></span>
                        </label>
                        <div style="text-align: center; margin-top: 8px; font-size: 12px; font-weight: 600;">
                            <span id="policy-status-text" style="color: ${currentSociety.requireCentralVerification ? '#28a745' : '#6c757d'};">
                                ${currentSociety.requireCentralVerification ? 'ON' : 'OFF'}
                            </span>
                        </div>
                    </div>
                </div>
...
function createAgentCard(agent) {
    ...
    // Load central verification status (non-blocking)
    let centralStatusHTML = '<span id="central-status-' + agent._id + '" style="font-size: 12px; color: #94a3b8;">⏳</span>';
    if (agent._id) {
        loadCentralStatus(agent._id);
    }
        ...
        <div class="agent-card" onclick="viewAgentDetails('${agent.email}')">
            <div style="display: flex; gap: 20px; align-items: center;">
                ...
                <div style="flex: 1;">
                    ...
                    <p style="color: #94a3b8; font-size: 12px; margin: 4px 0;">🔐 Central: ${centralStatusHTML}</p>
                </div>
...
// ============= CENTRAL VERIFICATION STATUS (READ-ONLY) =============
async function loadCentralStatus(agentId) {
    try {
        const response = await fetch(`${CONFIG.API_BASE_URL}/central-status/${agentId}`);
        const data = await response.json();
        
        const statusElement = document.getElementById('central-status-' + agentId);
        if (!statusElement) return;
        
        if (data.success && data.centralVerified) {
            const status = data.data.verificationStatus;
            const trustLevel = data.data.trustLevel;
            const riskScore = data.data.riskScore;
            
            // Create subtle status badge matching existing design
            let badgeHTML = '';
            let badgeColor = '';
            let badgeText = '';
            
            if (status === 'verified') {
                badgeColor = '#10b981'; // soft green
                badgeText = '✓ Verified';
            } else if (status === 'pending') {
                badgeColor = '#f59e0b'; // soft yellow
                badgeText = '⏳ Pending';
            } else if (status === 'flagged') {
                badgeColor = '#ef4444'; // soft red
                badgeText = '⚠️ Flagged';
            } else {
                badgeColor = '#94a3b8'; // neutral grey
                badgeText = status;
            }
            
            badgeHTML = `<span style="background: ${badgeColor}20; color: ${badgeColor}; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: 500;">${badgeText}</span>`;
            badgeHTML += ` <span style="color: #94a3b8; font-size: 11px;">(Risk: ${riskScore})</span>`;
            
            statusElement.innerHTML = badgeHTML;
        } else {
            // Not verified in central system
            statusElement.innerHTML = '<span style="color: #cbd5e1; font-size: 11px;">Not Linked</span>';
        }
    } catch (error) {
        console.error('Error loading central status:', error);
        const statusElement = document.getElementById('central-status-' + agentId);
        if (statusElement) {
            statusElement.innerHTML = '<span style="color: #cbd5e1; font-size: 11px;">—</span>';
        }
    }
}

// ============= CENTRAL VERIFICATION POLICY TOGGLE =============
async function toggleCentralVerificationPolicy(societyId) {
    const checkbox = document.getElementById('central-verification-toggle');
    const statusText = document.getElementById('policy-status-text');
    const enabled = checkbox.checked;
    
    // Show confirmation for enabling (to prevent accidental activation)
    if (enabled) {
        const confirmEnable = confirm(
            '⚠️ Enable Central Verification Policy?\n\n' +
            'When enabled, this policy will require agents to complete central verification.\n\n' +
    ...
    try {
        const response = await fetch(`${CONFIG.API_BASE_URL}/policy/central-verification/${societyId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
            ...
            if (currentSociety) {
                currentSociety.requireCentralVerification = enabled;
            }
            
            // Show success message
            const message = enabled 
                ? '✅ Central Verification Policy ENABLED\n\nAgents will now be required to complete central verification.'
