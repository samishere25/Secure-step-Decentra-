...
// Optional: Link central verification MAID to agent (passive, non-blocking)
router.post('/:agentId/link-central-verification', async (req, res) => {
  try {
    ...
    const agent = await Agent.findById(agentId) || await Agent.findOne({ email: agentId });
    ...
    // Soft-link: store MAID without affecting existing verification
    agent.centralVerificationId = maid;
    await agent.save();

    res.json({
      success: true,
      message: 'Central verification linked successfully',
      data: {
        agentId: agent._id,
        email: agent.email,
        centralVerificationId: agent.centralVerificationId
      }
    });
  } catch (error) {
    console.error('Error linking central verification:', error);
    res.status(500).json({ 
      success: false, 
      error: 'Failed to link central verification',
