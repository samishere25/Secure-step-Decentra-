...
class _GuardQRScannerScreenState extends State<GuardQRScannerScreen> {
  ...
  void _showResultDialog({
    ...
    showDialog(
      context: context,
      builder: (context) => _ResultDialogContent(
        isCheckIn: isCheckIn,
        isOffline: isOffline,
        agentName: agentName,
        company: company,
...
class _ResultDialogContentState extends State<_ResultDialogContent> {
  Map<String, dynamic>? _centralStatus;
  ...
  Future<void> _loadCentralStatus() async {
    setState(() => _loadingCentral = true);
    
    try {
      final response = await http.get(
        Uri.parse('${AppConstants.baseUrl}/api/central-status/${widget.agentId}'),
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (mounted && data['centralVerified'] == true) {
          setState(() => _centralStatus = data['data']);
        }
      }
    } catch (e) {
      ...
      if (mounted) {
        setState(() => _loadingCentral = false);
      }
    }
  }
    ...
    return AlertDialog(
      title: Row(
        children: [
          ...
          Expanded(child: Text(widget.isCheckIn ? 'CHECK-IN' : 'CHECK-OUT')),
          if (widget.isOffline)
            Container(
      ...
      content: Column(
        ...
        children: [
          Text('Name: ${widget.agentName}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
          ...
          Row(
            children: [
              Icon(
                widget.verified ? Icons.verified : Icons.warning,
                color: widget.verified ? Colors.green : Colors.orange,
                size: 20,
              ),
              const SizedBox(width: 8),
              Text(widget.verified ? 'Verified' : 'Not Verified'),
            ],
          ),
          const SizedBox(height: 12),
          // Central Verification Status (Informational Only)
          Container(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      else if (_centralStatus != null)
                        Text(
                          _centralStatus!['verificationStatus'] == 'verified'
                              ? 'Verified ✓'
                              : _centralStatus!['verificationStatus'] == 'pending'
                                  ? 'Pending'
                                  : 'Not Available',
                          style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w500,
                            color: _centralStatus!['verificationStatus'] == 'verified'
