...
function initMap() {
    map = L.map('map').setView([CONFIG.DEFAULT_CENTER.lat, CONFIG.DEFAULT_CENTER.lng], CONFIG.DEFAULT_ZOOM);
...
function updateMap() {
    ...
    alerts.forEach(alert => {
        ...
        if (lat && lng) {
            ...
            } else if (status === 'active' || status === 'triggered') {
            ...
            const popup = L.popup({
                maxWidth: 300,
                closeButton: true
            }).setContent(createMarkerPopup(alert));
...
async function dispatchPolice() {
    if (!selectedAlert) return;
    
    const societyName = typeof selectedAlert.societyId === 'string' ? selectedAlert.societyId : (selectedAlert.societyId?.name || 'Unknown');
    
    if (confirm(`Dispatch police unit to ${societyName} - Flat ${selectedAlert.flatNumber}?`)) {
        try {
            console.log('🚓 Dispatching police to:', selectedAlert.sosId);
            
            const response = await fetch(`${CONFIG.API_BASE_URL}/sos/${selectedAlert.sosId}/acknowledge`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    acknowledgedBy: 'Police Dashboard',
            ...
            if (response.ok && data.status === 'success') {
                ...
                updateAlertStatus(selectedAlert.sosId, 'acknowledged');
                closeModal();
            } else {
...
async function dispatchToAlert(sosId) {
    const alert = alerts.find(a => a.sosId === sosId);
    if (!alert) return;
    
    const societyName = typeof alert.societyId === 'string' ? alert.societyId : (alert.societyId?.name || 'Unknown');
    
    if (confirm(`Dispatch police unit to ${societyName} - Flat ${alert.flatNumber}?`)) {
        try {
            ...
            if (response.ok && data.status === 'success') {
                ...
                updateAlertStatus(sosId, 'acknowledged');
            } else {
                alert(`Failed to dispatch: ${data.message || 'Unknown error'}`);
            }
        } catch (error) {
